...
def _load_doc_cache(path: Path) -> list[list[float]] | None:
    if not path.is_file():
        return None
...
def _load_query_cache(path: Path) -> dict[str, list[float]] | None:
    if not path.is_file():
        return None
    raw = json.loads(path.read_text(encoding="utf-8"))
...
def _prepare_dense_caches(
    *,
    ...
    for name, provider in models.items():
        ...
        doc_vectors = None if force_reembed else _load_doc_cache(doc_cache)
        if doc_vectors is not None and len(doc_vectors) == len(texts):
            _log(progress, f"{name}: reusing doc cache {doc_cache}")
        elif require_cache:
            raise FileNotFoundError(
                f"missing dense doc cache for {name}: {doc_cache} "
                "(run stage dense-only first)"
            )
        else:
            ...
            _save_vector_cache(
                doc_cache,
                {"model": name, "count": len(doc_vectors), "vectors": doc_vectors},
            )
            _log(progress, f"{name}: wrote doc cache {doc_cache}")

        qmap = None if force_reembed else _load_query_cache(query_cache)
        if qmap is not None and all(example.query in qmap for example in dataset):
            ...
            raise FileNotFoundError(
                f"missing dense query cache for {name}: {query_cache} "
                "(run stage dense-only first)"
            )
        else:
            ...
            _save_vector_cache(query_cache, {"model": name, "queries": qmap})
...
def _rank_chunk_ids_by_dense(
    ...
    *,
    ...
    return [
        RetrievedChunk(chunk_id=index, score=score, payload={})
        ...
        def _retrieve(
            ...
            *,
            ...
            fused = reciprocal_rank_fusion(
                [dense_list, sparse_list], k=rrf_k, limit=fusion_pool
            )
            return _books_from_fused(
                [hit.chunk_id for hit in fused], _books, limit=candidate_limit
            )
...
def _load_sparse_doc_cache(path: Path) -> list[SparseVector] | None:
    if not path.is_file():
        return None
    raw = json.loads(path.read_text(encoding="utf-8"))
    vectors = raw.get("vectors")
...
def _load_sparse_query_cache(path: Path) -> dict[str, SparseVector] | None:
    if not path.is_file():
        return None
    raw = json.loads(path.read_text(encoding="utf-8"))
    mapping = raw.get("queries")
...
def _embed_bge_sparse_queries_with_progress(
    ...
    *,
    ...
    for index, example in enumerate(dataset, start=1):
        ...
        out[example.query] = SparseVector(
...
def _prepare_bge_sparse_caches(
    *,
    ...
    doc_sparse = None if force_reembed else _load_sparse_doc_cache(doc_cache)
    ...
    qmap = None if force_reembed else _load_sparse_query_cache(query_cache)
    if qmap is not None and all(example.query in qmap for example in dataset):
    ...
    *,
    ...
    def _bm25_rank(query: str) -> list[RetrievedChunk]:
        return _rank_chunk_ids_by_sparse(
            bm25.encode_query(query), doc_bm25, limit=fusion_pool
        )
    ...
    def _retrieve_bm25(query: str) -> list[int]:
        ...
        return _books_from_fused(
            [hit.chunk_id for hit in fused], book_ids, limit=candidate_limit
        )
